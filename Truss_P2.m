...
a_load=0; %0 for no axial stiffness, 1 for considering axial stiffness
...
nodes(:,1)=1:nnodes;
nodes(:,2:3)=[0 0;4000 0;12000 0];%1st Node is chosen as the origin
...
for i=1:nelems
    ...
    mi=elems(i,4)^3*elems(i,5)/12; %Moment of Inertia of element
...
if a_load==0
    for i=nnodes:-1:1
        ...
        kg(:,3*i-2)=[];
        f(3*i-2)=[];
        nzero_indices(nzero_indices==3*i-2)=[];
    end
    %Elimination Method
    for i=size(dispBC,1):-1:1
        %Slope
